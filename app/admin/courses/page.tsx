...
import { BookOpen, Star, Users, Search, ChevronRight } from 'lucide-react'
...
export default async function AdminCoursesPage({ searchParams }: Props) {
  ...
  const [courses, total] = await Promise.all([
    prisma.course.findMany({
      ...
      include: {
        department: {
          select: { name: true, faculty: { select: { university: { select: { shortName: true } } } } },
        },
        _count: { select: { professorCourses: true, sections: true } },
      },
      orderBy: [{ reviewCount: 'desc' }, { code: 'asc' }],
      skip: (page - 1) * perPage,
      take: perPage,
    }),
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">{total.toLocaleString()} courses total</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 mb-6 flex flex-wrap gap-3">
        <form className="flex-1 min-w-60 flex gap-2">
          ...
          {q && <Link href={`/admin/courses?uni=${uniSlug}`} className="px-4 py-2 border border-slate-200 text-sm text-slate-600 rounded-lg hover:bg-slate-50">Clear</Link>}
        </form>

        <div className="flex flex-wrap gap-2">
          <Link href="/admin/courses" className={`px-3 py-2 text-sm rounded-lg border font-medium ${!uniSlug ? 'bg-blue-600 text-white border-blue-600' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>All</Link>
          {universities.map(uni => (
            <Link key={uni.id} href={`/admin/courses?uni=${uni.slug}${q ? `&q=${q}` : ''}`}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50">
              ...
              <th className="text-left px-4 py-3 font-semibold text-slate-600">Course Name</th>
              ...
              <th className="text-center px-4 py-3 font-semibold text-slate-600 hidden sm:table-cell">Professors</th>
              <th className="text-center px-4 py-3 font-semibold text-slate-600 hidden sm:table-cell">Credits</th>
              ...
              return (
                <tr key={course.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-3">
                    <span className="font-mono text-xs font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded">{course.code}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="font-medium text-slate-900 text-sm">{course.name}</span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className="flex items-center justify-center gap-1 text-slate-600">
                      <Star className="h-3.5 w-3.5 text-slate-400" /> {course.reviewCount}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center hidden sm:table-cell">
                    <span className="flex items-center justify-center gap-1 text-slate-600">
                      <Users className="h-3.5 w-3.5 text-slate-400" /> {course._count.professorCourses}
                    </span>
                  </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="flex gap-2">
            {page > 1 && (
              <Link href={`/admin/courses?page=${page - 1}${q ? `&q=${q}` : ''}${uniSlug ? `&uni=${uniSlug}` : ''}`}
